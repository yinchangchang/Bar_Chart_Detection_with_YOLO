...
def image_to_numpy(image, input_shape):
    phase = args.phase
    image = np.array(image)
    image = image.transpose(2, 0, 1)
    if phase == 'test':
        size = max(image.shape[1:3])
        input_shape = [int(size / 128) * 128, int(size / 128) * 128]
        if input_shape[0] < size:
            input_shape[0] += 128
        if input_shape[1] < size:
            input_shape[1] += 128
    ...
    if shape[1] > h:
        ...
        pad = np.zeros((shape[0], h - shape[1], shape[2]))
        image = np.concatenate((image, pad), 1)
    shape = image.shape
    if shape[2] > w:
        image = image[:, :, :w]
    else:
        pad = np.zeros((shape[0], shape[1], w - shape[2]))
        image = np.concatenate((image, pad), 2)
    return image
...
def add_bbox_to_image(image, detected_bbox):
    ...
    for bbox in detected_bbox:
        xm, ym = image.shape[1:3]
        xmin, ymin, xmax, ymax = [int(x) for x in bbox[1:5]]
        xmin = max(0, min(xm-1, xmin))
        xmax = max(0, min(xm-1, xmax))
        ymin = max(0, min(ym-1, ymin))
        ymax = max(0, min(ym-1, ymax))
        image[:, xmin:xmax, ymin] = 0
        image[:, xmin:xmax, ymax] = 0
        image[:, xmin, ymin:ymax] = 0
        image[:, xmax, ymin:ymax] = 0
...
def augment(image, bbox, input_shape):
    assert image.shape[0] == 3
    hw = image.shape[1:]

    rx,ry,rn = np.random.random(3)

    center_list = []
    for i,b in enumerate(bbox):
        xmin,ymin,xmax,ymax = b[:4]
        if ymax - ymin < input_shape[1] and xmax - xmin < input_shape[0] and ymax - ymin > 1 and xmax - xmin > 1:
            center_list.append(i)
    rn = int(len(center_list) * rn)
    rn = center_list[rn]

    xmin,ymin,xmax,ymax = bbox[rn][:4]
    delta_x = max(input_shape[0] - (xmax - xmin), 0)
    delta_y = max(input_shape[1] - (ymax - ymin), 0)
    sx = min(max(0, int(xmin - rx * delta_x)), hw[0] - input_shape[0])
    sy = min(max(0, int(ymin - ry * delta_y)), hw[1] - input_shape[1])
    ex = input_shape[0] + sx
    ey = input_shape[1] + sy
    ...
    for i,b in enumerate(bbox):
        ...
        xmin = max(sx, min(ex, xmin)) - sx
        xmax = max(sx, min(ex, xmax)) - sx
        ymin = max(sy, min(ey, ymin)) - sy
        ymax = max(sy, min(ey, ymax)) - sy
        if i == rn:
            try:
        ...
        if xmax > xmin and ymax > ymin:
            nbl.append([xmin, ymin, xmax, ymax, l])
    bbox = nbl
