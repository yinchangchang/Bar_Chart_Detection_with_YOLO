...
import loaddata
import framework
from loaddata import segloader
...
args.neg_th = 0.8       # nms threshold 最小概率
args.nms_ol = 0.1       # ns max overlap 最大重叠
args.gpu = 1

args.anchors = []
args.input_shape = [256, 256]
x_delta_list = [48, 96, 128, 256]
x_delta_list = [4, 6, 8, 16]
y_delta_list = [3, 12, 32, 64]
for x in x_delta_list:
    args.anchors.append([x, x])
...
def test(p_dict, have_label=0):
    ...
    for i,data in enumerate(tqdm(data_loader)):
        ...
        if args.use_detection:
            ...
            function.save_detection_results(data[0], images.data.cpu().numpy(), detect_character_output.data.cpu().numpy())
...
def main():
    ...
    test_dataset = segloader.DataSet(
                test_filelist, 
                image_label_dict,
                num_classes, 
    ...
    test_loader = DataLoader(
                ...
                pin_memory=True)
    train_dataset  = segloader.DataSet(
                train_filelist, 
                image_label_dict, 
                num_classes, 
                image_size=image_size,
    ...
    val_loader = DataLoader(
                ...
                pin_memory=True)
    '''
    pretrain_dataset = segloader.DataSet(
                pretrain_filelist, 
                image_label_dict,
                num_classes, 
                image_size=image_size,
    ...
    pretrain_loader = DataLoader(
                ...
                num_workers=args.workers, 
                pin_memory=True)
    preval_dataset = segloader.DataSet(
                preval_filelist, 
                image_label_dict,
                num_classes, 
                image_size=image_size,
